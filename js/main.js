    ...
    if (hash && hash.startsWith('#share=')) {
        ...
        try {
            ...
            setTimeout(() => {
                ...
                // Validate Expiration (1 Hour = 3600000 ms)
                const now = Date.now();
                if (now - shareData.timestamp > 3600000) {
                    throw new Error("Bu paylaşım linkinin süresi dolmuş (1 saat geçerli).");
                }
                
                // Load Data
                originalJson = shareData.data;
                analyzeAndProcess(originalJson);
                
                // Switch to Viewer Mode
                enableSharedViewMode();
...
function enableSharedViewMode() {
    ...
    if (title) {
        title.innerHTML += ' <span class="bg-indigo-100 text-indigo-800 text-xs font-semibold px-2.5 py-0.5 rounded ml-2">Paylaşım Modu</span>';
    }
